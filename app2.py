...
# -------------------- ML ÖNERİ SİSTEMİ --------------------
    ...
    st.write("Bu bölümde, veri setinizin özelliklerine göre hangi makine öğrenmesi modellerinin kullanılabileceği, izlenmesi gereken metrikler ve uygulanması gereken adımlar hakkında öneriler sunulacaktır. (Model eğitimi veya sonuç hesaplaması yapılmayacaktır.)")
    ...
    if yuklenen_veri is not None:
        ...
        st.subheader("Adım 1: Veri Temizleme & Ön İşleme")
        st.write("""
        - **Eksik Verilerin İmputasyonu:** 
            - Sayısal değişkenler için ortalama, medyan veya mod;
            - Kategorik değişkenler için en sık görülen değer (mod) veya belirli bir sınıf ile doldurma.
        - **Aykırı Değerlerin İncelenmesi:** 
            - Boxplot veya scatter plot gibi grafiklerle tespit;
            - Gerekirse, aykırı değerlerin düzeltilmesi ya da veri setinden çıkarılması.
        - **Özellik Ölçekleme:** 
            - Standardizasyon (Z-score) veya Min-Max normalizasyonu,
            - Özellikle mesafe bazlı modeller için önemlidir.
        - **Veri Dönüşümleri:** 
            - Log, karekök veya Box-Cox dönüşümleri ile verinin dağılımını düzenleme.
        """)

        st.subheader("Adım 2: Model Türüne Karar Verme")
        hedef_deg = st.selectbox("Hedef değişkeni seçiniz:", veri.columns)
        if pd.api.types.is_numeric_dtype(veri[hedef_deg]):
            st.write("Hedef değişken sayısal görünüyor. Önerilen regresyon modelleri:")
            st.markdown("""
            **Önerilen Modeller:**
            - Lineer Regresyon
            - Ridge Regresyon
            ...
            **Önerilen Metrikler:**
            ...
            st.write("Hedef değişken kategorik görünüyor. Önerilen sınıflandırma modelleri:")
            st.markdown("""
            **Önerilen Modeller:**
            - Lojistik Regresyon
            - KNN Sınıflandırma
        ...
        st.subheader("Adım 3: Model Eğitimi Öncesi")
        st.write("""
        - **Veri Setini Bölme:** 
            - Eğitim ve test verisi ayrımı (örneğin %80 eğitim, %20 test).
            - Gerekirse doğrulama seti oluşturulabilir.
        - **Hiperparametre Ayarları:** 
            - Grid Search, Random Search veya Bayesian Optimization yöntemleriyle optimize edin.
        - **Çapraz Doğrulama (Cross-Validation):** 
            - K-fold CV ile modelin genelleme kabiliyetini değerlendirin.
        - **Özellik Seçimi:** 
            - Gereksiz veya düşük katkılı değişkenleri elemek,
            - Model karmaşıklığını azaltarak overfitting riskini düşürmek.
        """)

        st.subheader("Özet")
        st.write("""
        - **Veri Temizleme & Ön İşleme:** Eksik veriler, aykırı değerler ve ölçekleme işlemleriyle verinizi analiz için hazırlayın.
        - **Model Seçimi:** Hedef değişkenin türüne bağlı olarak regresyon veya sınıflandırma modelleri arasından seçim yapın.
        - **Model Eğitimi Öncesi:** Eğitim-test ayrımı, hiperparametre optimizasyonu ve çapraz doğrulama adımlarıyla modelinizin performansını artırmayı hedefleyin.
        - Bu adımları izleyerek veri setinize en uygun  modeli seçip sonraki eğitim ve optimizasyon aşamalarına geçebilirsiniz.
        """)
